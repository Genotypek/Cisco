...
GUICtrlSetState($GUI_Cisco_Radio_ObrazekGora, $GUI_CHECKED)
...
Func DodajPytaniaDoTablicy($Schowek) ; Dodawanie pytañ do tablicy
	...
	For $i = 0 To UBound($Array_Tablica_Tymczasowa) - 2 Step 1 ; Dodawanie par s³ówek do tablicy w³aœciwej, oddzielonych "€"
		...
		$Array_Tablica_Pytan[UBound($Array_Tablica_Pytan) - 1][1] = "• " & $Array_Tablica_Pytan[UBound($Array_Tablica_Pytan) - 1][1]
		$Array_Tablica_Pytan[UBound($Array_Tablica_Pytan) - 1][1] = StringReplace($Array_Tablica_Pytan[UBound($Array_Tablica_Pytan) - 1][1], "&", @CRLF & "• ")
	Next
...
;### Tidy Error: If/ElseIf statement without a then..
	If __IsChecked($GUI_Cisco_Radio_ObrazekGora) Then

	ElseIf __IsChecked($GUI_Cisco_Radio_ObrazekLewo) Then

	ElseIf __IsChecked($GUI_Cisco_Radio_ObrazekPrawo) Then

	ElseIf __IsChecked($GUI_Cisco_Radio_ObrazekDol Then

		$x = ($PosCisco[0] + ($PosCisco[2] / 2)) - $PosIlustracje[2] / 2
		$y = $PosCisco[1] + $PosCisco[3]
	EndIf
		WinMove($GUI_Ilustracja, "", $x, $y)
;### Tidy Error -> "endfunc" is closing previous "if" on line 298
